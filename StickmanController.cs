...
public class StickmanController : MonoBehaviour
{
    ...
    void MoveLimbs()
    {
        // Move Left Arm with 'Q'
        if (Input.GetKeyDown(KeyCode.Q))
        {
            leftArm.Rotate(0f, 0f, 15f); // Rotate the left arm
            PlayLimbSound(); // Play sound
        }

        // Move Right Arm with 'E'
        if (Input.GetKeyDown(KeyCode.E))
        {
            rightArm.Rotate(0f, 0f, -15f); // Rotate the right arm
            PlayLimbSound(); // Play sound
        }

        // Move Left Leg with 'Z'
        if (Input.GetKeyDown(KeyCode.Z))
        {
            leftLeg.Rotate(0f, 0f, 15f); // Rotate the left leg
            PlayLimbSound(); // Play sound
        }

        // Move Right Leg with 'C'
        if (Input.GetKeyDown(KeyCode.C))
        {
            rightLeg.Rotate(0f, 0f, -15f); // Rotate the right leg
            PlayLimbSound(); // Play sound
        }
    }
    ...
    void PlayLimbSound()
    {
        audioSource.PlayOneShot(limbMoveSound); // Play sound
    }
}
